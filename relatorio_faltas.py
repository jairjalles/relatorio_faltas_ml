...
# Autor: Jair Jales - Versão Profissional com Filtro por Conta e Marca
...
planilha = c_path.text_input(
    "", "planilhas/FALTAS MERCADO LIVRE 2025.xlsx", key="input_path"
)
...
with tabs[0]:
    ...
    # Filtros por Conta e Marca
    contas_unicas = ["Todas"] + sorted(df_faltas["Conta_Exibicao"].unique())
    marcas_unicas = ["Todas"] + sorted(df_long["Marca"].dropna().unique())

    st.markdown(
        """<div style='background-color:rgba(255,255,255,0.07);border-radius:15px;padding:20px;margin-bottom:20px;'>""",
        unsafe_allow_html=True
    )
    f_acc, f_marca = st.columns(2)
    with f_acc:
        conta_sel = st.selectbox("📁 Filtrar por Conta", contas_unicas, key="filter_conta")
    with f_marca:
        marca_sel = st.selectbox("🏷️ Filtrar por Marca", marcas_unicas, key="filter_marca")
    st.markdown("</div>", unsafe_allow_html=True)

    # Aplica filtros
    df_filtrado = df_long.copy()
    if conta_sel != "Todas":
        df_filtrado = df_filtrado[df_filtrado["Conta_Exibicao"] == conta_sel]
    if marca_sel != "Todas":
        df_filtrado = df_filtrado[df_filtrado["Marca"] == marca_sel]
    tot_hoje = int(df_filtrado["Faltas"].sum())
    ...
    # SKU crítico
    imp = (
        df_filtrado[df_filtrado["Faltas"]==1]
        .groupby("SKU")["Conta_Exibicao"].count()
        .reset_index(name="Qtd")
        .sort_values("Qtd", ascending=False)
    )
    if not imp.empty:
        top = imp.iloc[0]
        msg_imp = f"⚠️ SKU {top['SKU']} em falta em {top['Qtd']} contas"
    else:
        msg_imp = "✅ Nenhum SKU crítico hoje"

    # Total SKUs com falta hoje
    skus_hj = df_filtrado[df_filtrado["Faltas"]==1]["SKU"].nunique()
    ...
    <div style='display:flex;gap:20px;flex-wrap:wrap;margin-bottom:30px;'>
      ...
      <div class='custom-card'><h3>🏬 Contas Ativas</h3><p style='font-size:26px;font-weight:bold;'>{df_filtrado['Conta_Exibicao'].nunique()}</p></div>
    ...
    top_m = (
        df_filtrado.groupby("Marca")["Faltas"].sum()
        .reset_index().sort_values("Faltas", ascending=False).head(10)
    )
    g2 = px.bar(
...
with tabs[2]:
    ...
    sa = (
        df_filtrado[df_filtrado["Faltas"]==1]
        .groupby("SKU")["Conta_Exibicao"].count()
        .reset_index(name="Contas").query("Contas>=5")
    )
