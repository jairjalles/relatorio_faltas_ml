# dashboard_faltas_final.py
# Autor: Jair Jales — Versão Profissional Enxuta com Filtro por Conta e Marca
...
planilha = path_col.text_input("", "planilhas/FALTAS MERCADO LIVRE 2025.xlsx", key="input_path")
if btn_col.button("🔄 Atualizar", key="btn_refresh"):
...
with tabs[0]:
    if df_long.empty:
        ...
        # --- FILTROS ---
# dentro da aba 0, logo antes de aplicar o df_filtrado
st.markdown(
...
# preenchemos as opções filtrando NaNs e convertendo para str
...
marca_sel = fcol2.selectbox("🏷️ Filtrar por Marca", marca_opts, key="filtro_marca")
st.markdown("</div>", unsafe_allow_html=True)

# depois, aplicamos o filtro ao DataFrame
df_fil = df_long.copy()
if conta_sel != "Todas":
...
if marca_sel != "Todas":
    df_fil = df_fil[df_fil["Marca"] == marca_sel]
        ...
        skus_hj = df_fil[df_fil["Faltas"]==1]["SKU"].nunique()
...
with tabs[3]:
    ...
    st.download_button("⬇️ Detalhadas", df_long.to_csv(index=False).encode(), file_name="detalhadas.csv", key="e2")
